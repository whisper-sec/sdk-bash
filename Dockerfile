...
ADD whisper-api-cli /usr/bin/whisper-api-cli
ADD _whisper-api-cli /usr/local/share/zsh/site-functions/_whisper-api-cli
ADD whisper-api-cli.bash-completion /etc/bash-completion.d/whisper-api-cli
RUN chmod 755 /usr/bin/whisper-api-cli
...
RUN echo '\n\
. /etc/bash_completion\n\
source /etc/bash-completion.d/whisper-api-cli\n\
' >> ~/.bashrc

#
# Setup prompt
#
RUN echo 'export PS1="[Whisper API v1] \$ "' >> ~/.bashrc
RUN echo 'export PROMPT="[Whisper API v1] \$ "' >> ~/.zshrc
...
line REST client for $(tput setaf 6)Whisper API v1$(tput sgr0).\n\
\n\
